 ...
 * Client Fetch
 *
 * Proxies an API request through the client action:
 * 1. Reads the encrypted session from localStorage
 ...
 */
export async function clientFetch<TData = unknown>(
  ...
  try {
    const response = await makeRequest<TData>(config, session?.token || '');
    ...
    if (
      ...
      if (isRefreshTokenEnabled()) {
        ...
        const retryResponse = await makeRequest<TData>(config, newSession.token || '');
